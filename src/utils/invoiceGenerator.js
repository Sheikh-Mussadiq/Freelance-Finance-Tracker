...
export const generateInvoice = (project, payments, companyInfo) => {
  ...
  doc.text(
    `Contract Type: ${
      project.contractType.charAt(0).toUpperCase() +
      project.contractType.slice(1)
    }`,
    20,
    105
  );
  ...
  doc.text(
    `Start Date: ${format(new Date(project.startDate), "MMM d, yyyy")}`,
    20,
    115
  );
  if (project.endDate) {
    doc.text(
      `End Date: ${format(new Date(project.endDate), "MMM d, yyyy")}`,
      20,
      120
    );
  }
  ...
  const tableY = (doc.lastAutoTable || {}).finalY || 150;

  // Add payment instructions
  doc.setFontSize(10);
  doc.text("Payment Instructions:", 20, tableY + 20);
  doc.text(
    companyInfo.paymentInstructions || "Please make payment within 30 days.",
    20,
    tableY + 30
  );
